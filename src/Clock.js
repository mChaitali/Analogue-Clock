...
function Clock() {
  ...
  return (
    <div className="clock">
      ...
      style={{transform: `rotate(${(time.getHours() + time.getMinutes()/60) * 30}deg)`}}></div>
      <div className='minute_hand'
      style={{transform: `rotate(${time.getMinutes() * 6}deg)`}}></div>
      <div className='second_hand'
      style={{transform: `rotate(${time.getSeconds() * 6}deg)`}}></div>
    </div>
  );
}
